...
* **智能过滤**：距离过滤随震级放宽（M4 → 250km … M10 → 6000km）、低烈度过滤、
  盲区兜底（近震中立即提示"已到达"）
* **倒计时语音（可选资源包）**：触发播预警音（按国标蓝/黄/橙/红）+ 逐秒中文报数，语音素材为公开渠道收集的提示音（来源见文末免责声明，非小米官方素材）；不装资源包则无声，不影响倒计时
...
预警音播完后**逐秒中文报数**剩余秒数。语音素材（数字 1~10、间隔音、预警音）为公开渠道收集的
提示音（具体来源见文末免责声明），以 `MCEEW-yujing-sounds` 资源包形式提供（Release 附件）。
...
* **仅供游戏娱乐**：本插件的倒计时、烈度、预警等级均为估算模拟（衰减公式/分级口径参考
  小米手机地震预警与国家标准的近似复刻），**不是真实的地震预警服务**，不能用于实际防灾避险。
* **数据源**：地震信息来自 Wolfx 等第三方聚合接口（见上"API Source"），不保证实时性、
  完整性与准确性；IP 定位使用腾讯位置服务及 ip2region/GeoNames 数据，存在误差。
  GeoNames 数据遵循 CC BY 4.0（© GeoNames contributors，https://www.geonames.org/），
  ip2region 遵循 Apache-2.0（https://github.com/lionsoul2014/ip2region）。
* **语音素材**：预警语音/数字提示音为公开渠道收集的音频（数字 1~10、间隔音及预警提示音，
  来源与原作者未知，经格式转码后仅用于游戏内体验），不保证原创性；若素材涉及您的版权，
  请告知，作者将立即从资源包与 Release 中移除。本插件与小米/相关机构无任何隶属或合作关系。
* **安全提醒**：如身处真实地震高风险区域，请以官方渠道（国家地震台网、手机系统地震预警等）
  为准，本插件**不能**替代任何真实预警手段。
* **使用风险**：本插件按"现状"提供，作者不对因使用造成的任何损失负责。生产服务器部署前请
  自行测试（建议先用 `/eew test region ... <玩家名>` 定向验证）。
* **上游与许可**：基于 TenkyuChimata/MCEEW（GPL-3.0）fork；本 fork 修改遵循原项目开源许可。
